...
public class MasUserController {
    ...
    public ResponseEntity<BaseResponse<UserProfileResponse>> getMyProfile() {
        // ดึง ID ของคนที่ Login อยู่ ออกมาจาก SecurityContext (ที่ JwtFilter ยัดไว้ให้)
        //  UUID userId = (UUID) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        UserPrincipal currentUser = UserUtil.getCurrentUser();

        UserProfileProjection userProfile = masUserService.getUserProfileById(currentUser.getUserId());

        // ประกอบร่าง DTO ส่งกลับไปให้หน้าบ้าน
